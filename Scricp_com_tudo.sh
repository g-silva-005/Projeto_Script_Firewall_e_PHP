...
while true; do
        ...
        echo " [B] Ativar ou Desativar portas de firewall      [Q] Terminate"
        echo " [C] MariaDB Overview"
        echo " ---------------------------------------------------"
        read -p "SYSTEM_ROOT@CENTOS> " acao
        case $acao in
			a|A) 
...
END
          ...
          c) sudo mariadb -u root -e "STATUS; SHOW DATABASES;" && read -p "! Pressiona ENTER para continuar";;
          q|Q) exit ;;
                *) ;;
        esac
done
